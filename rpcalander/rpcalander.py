...
from .file_utils import read_last_posted
...
class RPCAGroup(app_commands.Group):
    ...
    @app_commands.command(
        name="info",
        description="View the current settings for the RP calendar.",
    )
    async def info(self, interaction: discord.Interaction) -> None:
        ...
        try:
            ...
            if current_date != "Not set":
                current_date_obj = datetime.strptime(current_date, "%m-%d-%Y").replace(tzinfo=timezone.utc)
        ...
        embed.add_field(
            ...
            value=time_until_next_post_str,
            inline=False,
        )
    ...
    @app_commands.command(
        name="force",
        description="Force post a calendar update to the configured channel immediately.",
    )
    ...
    @app_commands.command(
        name="settitle",
        description="Set a custom title for the main embed.",
    )
    ...
    @app_commands.command(
        name="setcolor",
        description="Set the embed color for calendar updates.",
    )
    async def setcolor(self, interaction: discord.Interaction, color: str) -> None:
        ...
        await interaction.followup.send(
            f"Embed color set to: #{color_str.zfill(6)}",
            ephemeral=True,
        )

    @app_commands.command(
        name="settimezone",
        description="Set the timezone for the calendar.",
    )
    ...
    @app_commands.command(
        name="setchannel",
        description="Set the channel for daily calendar updates.",
    )
    async def setchannel(
        ...
        await interaction.followup.send(
            f"Calendar updates will now be sent to: {channel.mention}",
            ephemeral=True,
        )

    @app_commands.command(
        name="togglefooter",
        description="Toggle the footer on/off for calendar embeds.",
    )
    ...
    @app_commands.command(
        name="moonphase",
        description="View the current moon phase for the RP calendar date.",
    )
    async def moonphase(self, interaction: discord.Interaction) -> None:
        ...
        if not guild_settings.get("show_moon_phase", False):
            await interaction.followup.send(
                "Moon phase tracking is not enabled. An administrator can enable it with `/rpca moonconfig enable`.",
                ephemeral=True,
            )
            return
        ...
        if not current_date_str:
            await interaction.followup.send(
                "The RP calendar has not been set up yet. Please set a start date first.",
                ephemeral=True,
            )
            return
        ...
        try:
            ...
            current_date = datetime.strptime(current_date_str, "%m-%d-%Y").replace(tzinfo=timezone.utc)

            # Get moon data and create embed
            from .moon_utils import create_moon_embed, get_moon_data
    ...
    @app_commands.command(
        name="forcemoonupdate",
        description="Force post a moon phase update to the configured channel.",
    )
    ...
    async def forcemoonupdate(self, interaction: discord.Interaction) -> None:
        ...
        if not guild_settings.get("show_moon_phase", False):
            await interaction.followup.send(
                "Moon phase tracking is not enabled. Enable it with `/rpca moonconfig enable`.",
                ephemeral=True,
            )
            return
        ...
        try:
            ...
            await interaction.followup.send(
                "Moon phase update has been posted.",
                ephemeral=True,
            )
        except RECOVERABLE_EXCEPTIONS as e:
            await interaction.followup.send(
    ...
    @app_commands.command(
        name="moonconfig",
        description="Configure moon phase settings for the RP calendar.",
    )
    @app_commands.default_permissions(administrator=True)
    ...
    async def moonconfig(
        ...
        if action == "enable":
            ...
            await interaction.followup.send(
                "Moon phase tracking has been enabled.",
                ephemeral=True,
            )
            ...
            await interaction.followup.send(
                "Moon phase tracking has been disabled.",
                ephemeral=True,
            )
            ...
            await self.cog._config.guild(interaction.guild).blood_moon_enabled.set(
                new_setting,
            )

            if new_setting:
                await interaction.followup.send(
                    "🔴 **BLOOD MOON MODE ACTIVATED!** The moon may now occasionally turn blood red during full moons!",
                    ephemeral=False,
                )
            else:
                await interaction.followup.send(
                    "Blood moon mode has been disabled.",
                    ephemeral=True,
                )
            ...
            if not channel:
                await interaction.followup.send(
                    "Please specify a channel for moon phase updates.",
                    ephemeral=True,
                )
                return

            await self.cog._config.guild(interaction.guild).moon_channel_id.set(
                channel.id,
            )
            await interaction.followup.send(
                f"Moon phase updates will now be sent to: {channel.mention}",
                ephemeral=True,
            )
    ...
    @app_commands.command(
        name="setdate",
        description="Set the current RP calendar date.",
    )
    @app_commands.default_permissions(administrator=True)
    async def setdate(self, interaction: discord.Interaction, date: str = "") -> None:
        ...
        if not date:
            ...
            try:
                datetime.strptime(date, "%m-%d-%Y").replace(tzinfo=timezone.utc)
            except ValueError:
                await interaction.response.send_message(
                    "Invalid date format. Please use MM-DD-YYYY (e.g., 05-15-2023)",
                    ephemeral=True,
                )
                return
...
class RPCalander(DashboardIntegration, commands.Cog):
    """
    ...
    def __init__(self, bot: commands.Bot) -> None:
        ...
        self._default_guild = {
            ...
            # Separate channel for moon phase updates (defaults to same as calendar)
            "moon_channel_id": None,
        }
    ...
    async def cog_load(self):
        ...
        for guild_id, guild_settings in all_guilds.items():
            ...
            if current_date:
                ...
                today_date_obj = datetime.now(tz).replace(
                    hour=0,
                    minute=0,
                    second=0,
                    microsecond=0,
    ...
    async def _daily_update_loop(self) -> None:
        ...
        for guild_id, guild_settings in all_guilds.items():
            ...
            if last_posted:
                ...
                last_posted_dt = now - timedelta(days=1)
            next_post_time = last_posted_dt.replace(
                hour=0,
                minute=0,
                second=0,
                microsecond=0,
            ...
            if now >= next_post_time:
                ...
                await self._config.guild_from_id(guild_id).last_posted.set(
                    now.isoformat(),
                )
                embed = discord.Embed(
                    title=embed_title,
    ...
    async def force_post(self, guild: discord.Guild) -> tuple[bool, str]:
        ...
        try:
            ...
            if not self._is_same_month_day(current_date_obj, today_date_obj):
                ...
                new_date_obj = current_date_obj
            new_date_str = new_date_obj.strftime("%A %m-%d-%Y")
            await self._config.guild(guild).current_date.set(
                new_date_obj.strftime("%m-%d-%Y"),
            )
    ...
    async def _post_moon_update(self, guild) -> None:
        ...
        try:
            ...
            moon_channel_id = guild_settings.get(
                "moon_channel_id",
            ...
            if moon_data.get("is_blood_moon", False):
                await channel.send(
                    "@everyone **A Blood Moon has risen! Strange energies fill the air...**",
                )
    ...
    @commands.guild_only()
    @commands.admin_or_permissions(administrator=True)
    async def rpca_moonconfig_setchannel(
        self,
        ctx: commands.Context,
