...
class DashboardIntegration:
    ...
    def _dashboard_active_tab(self, kwargs: dict[str, typing.Any]) -> str:
        form_data = self._dashboard_form_data(kwargs)
    ...
    async def _dashboard_handle_action(
        ...
        if action == "save_autoroles":
            ...
            messages.append(
                {"message": "Autorole settings saved.", "category": "success"},
            )
            return selected_role_id, messages
        ...
        if action == "create_reaction_panel":
            message = await self._dashboard_create_reaction_panel(
                guild,
                member,
                form_data,
            )
        ...
        if action == "create_button":
            ...
            messages.append(
                {"message": f"Button `{name}` saved.", "category": "success"},
            )
            return selected_role_id, messages
        ...
        if action == "delete_button":
            ...
            messages.append(
                {"message": f"Button `{name}` deleted.", "category": "success"},
            )
            return selected_role_id, messages
        ...
        if action == "sticky_member_action":
            message = await self._dashboard_sticky_member_action(
                guild,
                member,
                form_data,
            )
    ...
    def _dashboard_check_role_manageable(
        ...
        if role >= me.top_role:
            raise commands.BadArgument(
                f"The bot's top role must be above `{role.name}`.",
            )
        if (
            member is not None
    ...
    async def _dashboard_create_role(
        ...
        if len(guild.roles) >= 250:
            raise commands.BadArgument(
                "This server is already at Discord's 250 role limit.",
            )
        me = guild.me
        if me is None or not me.guild_permissions.manage_roles:
            raise commands.BadArgument("The bot needs Manage Roles.")
    ...
    async def _dashboard_save_role_rule(
        ...
        if not add_ids and not remove_ids:
            raise commands.BadArgument(
                "A role rule needs at least one add or remove action.",
            )
        if trigger.id in add_ids or trigger.id in remove_ids:
            raise commands.BadArgument(
                "A role rule cannot modify its own trigger role.",
            )
    ...
    async def _dashboard_save_role_flags(
        ...
        if role.id in required_ids + inclusive_ids + exclusive_ids:
            raise commands.BadArgument(
                "A role cannot require, include, or exclude itself.",
            )
        ...
        await self.config.role(role).require_any.set(
            self._dash_bool(form_data, "require_any"),
        )
    ...
    async def _dashboard_save_autoroles(
        ...
        settings = {
            ...
            "humans": self._dashboard_valid_role_ids(
                guild,
                member,
                form_data,
                "auto_humans",
            ),
            "bots": self._dashboard_valid_role_ids(
                guild,
                member,
                form_data,
                "auto_bots",
    ...
    async def _dashboard_give_temp_role(
        ...
        if target is None:
            raise commands.BadArgument(
                "Temporary-role member was not found in this server.",
            )
    ...
    async def _dashboard_role_operation(
        ...
        if policy_added - {role}:
            extras.append(
                "also add "
                + self._dashboard_role_names(
                    guild,
                    [item.id for item in policy_added - {role}],
                ),
            )
        if policy_removed - {role}:
            extras.append(
                ...
                + self._dashboard_role_names(
                    guild,
                    [item.id for item in policy_removed - {role}],
                ),
            )
    ...
    async def _dashboard_create_reaction_panel(
        ...
        for part in parts:
            ...
            if not emoji or role is None:
                raise commands.BadArgument(
                    f"Could not resolve reaction binding `{part}`.",
                )
            self._dashboard_check_role_manageable(guild, role, member)
        ...
        for emoji, role in bindings:
            try:
                ...
                raise commands.BadArgument(
                    f"Discord rejected reaction `{emoji}`.",
                ) from exc
    ...
    async def _dashboard_refresh_reaction_message(
        ...
        try:
            ...
            raise commands.BadArgument(
                "The reaction-role message could not be fetched.",
            ) from exc
        refreshed = 0
    ...
    async def _dashboard_create_select_menu(
        ...
        try:
            ...
            raise commands.BadArgument(
                "Menu min/max values must be whole numbers.",
            ) from exc
    ...
    async def _dashboard_send_component_message(
        ...
        if not isinstance(channel, discord.TextChannel):
            raise commands.BadArgument(
                "Component message channel must be a text channel.",
            )
        button_names = self._parse_name_list(
            self._dash_value(form_data, "component_buttons"),
        )
        select_names = self._parse_name_list(
            self._dash_value(form_data, "component_selects"),
        )
    ...
    async def _dashboard_edit_component_message(
        ...
        try:
            ...
            raise commands.BadArgument(
                "I could not fetch that component message.",
            ) from exc
    ...
    async def _dashboard_source(
        ...
        <style>
            ...
            .rmdash-row {{ display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 12px;
            margin-bottom: 12px; }}
            .rmdash-card {{ background: #1f2937; border: 1px solid #374151; border-radius: 8px; padding: 16px;
            margin-bottom: 16px; }}
            ...
            .rmdash-btn {{ background: #2563eb; color: white; border: 0; border-radius: 6px; padding: 9px 14px; cursor:
            ...
            .rmdash-table th, .rmdash-table td {{ border-bottom: 1px solid #374151; padding: 8px; text-align: left;
            vertical-align: top; }}
        ...
        <div class="rmdash-wrap" data-rmdash-tabs="1">
            <div class="rmdash-card">
                ...
                <div class="rmdash-grid">
                    <div><div class="rmdash-muted">Self Roles</div><div
                    class="rmdash-stat">{role_stats["self_roles"]}</div></div>
                    <div><div class="rmdash-muted">Sticky Roles</div><div
                    class="rmdash-stat">{role_stats["sticky_roles"]}</div></div>
                    <div><div class="rmdash-muted">Default Temp Roles</div><div
                    class="rmdash-stat">{role_stats["temp_roles"]}</div></div>
                    <div><div class="rmdash-muted">Reaction Bindings</div><div
                    class="rmdash-stat">{reaction_bind_count}</div></div>
                    <div><div class="rmdash-muted">Buttons</div><div class="rmdash-stat">{len(buttons)}</div></div>
                    <div><div class="rmdash-muted">Select Menus</div><div
                    class="rmdash-stat">{len(select_menus)}</div></div>
                    <div><div class="rmdash-muted">Pending Temp Roles</div><div
                    class="rmdash-stat">{len(temp_roles)}</div></div>
                    <div><div class="rmdash-muted">Role Rules</div><div
                    class="rmdash-stat">{len(role_rules)}</div></div>
                </div>
            </div>
            ...
            id="rmdash-panel-roles" role="tabpanel" aria-labelledby="rmdash-tab-roles">
            ...
            <section class="rmdash-tab-panel{self._dashboard_active_class("panels", active_tab)}"
            data-tab-panel="panels" id="rmdash-panel-panels" role="tabpanel" aria-labelledby="rmdash-tab-panels">
    ...
    def _dashboard_role_rules_section(
        self,
        guild: discord.Guild,
        rules: dict[str, dict[str, typing.Any]],
        csrf: str,
    ) -> str:
        rows = []
        editors = []
        for name, rule in sorted(rules.items()):
            ...
            trigger_event_select = self._select(
                "rule_event",
                "Trigger Event",
                [("add", "Role added"), ("remove", "Role removed")],
                rule.get("trigger_event", "add"),
            )
            trigger_role_select = self._role_select(
                guild,
                "rule_trigger_role_id",
                "Trigger Role",
                rule.get("trigger_role_id"),
            )
            add_roles_select = self._multi_role_select(
                guild,
                "rule_add_roles",
                "Roles To Add",
                rule.get("add_role_ids", []),
            )
            remove_roles_select = self._multi_role_select(
                guild,
                "rule_remove_roles",
                "Roles To Remove",
                rule.get("remove_role_ids", []),
        ...
        new_trigger_event_select = self._select(
            "rule_event",
            "Trigger Event",
            [("add", "Role added"), ("remove", "Role removed")],
            "add",
    ...
    def _dashboard_role_operations_section(
        self,
        guild: discord.Guild,
        csrf: str,
    ) -> str:
        operation_select = self._select(
            "operation",
            "Operation",
            [("add", "Add role"), ("remove", "Remove role")],
            "add",
        )
        target_type_select = self._select(
            "operation_target_type",
            "Target Type",
            [
                ("member", "Member ID"),
                ("role", "Members with role ID"),
                ("channel", "Members in channel ID"),
                ("everyone", "Everyone"),
                ("humans", "Humans"),
                ("bots", "Bots"),
            ],
    ...
    def _dashboard_sticky_section(self, guild: discord.Guild, csrf: str) -> str:
        sticky_operation_select = self._select(
            "sticky_operation",
            "Operation",
            [
                ("add", "Add and remember"),
                ("remove", "Remove and forget"),
                ("forget", "Forget departed user"),
            ],
            "add",
        )
        return f"""
    ...
    def _dashboard_import_section(self, csrf: str) -> str:
        import_source_select = self._select(
            "import_source",
            "Source Cog",
            [("roletools", "Trusty RoleTools"), ("roleutils", "Seina RoleUtils")],
            "roletools",
        )
        return f"""
    ...
    async def _dashboard_role_stats(self, guild: discord.Guild) -> dict[str, int]:
        ...
        if selected_role is None:
            ...
            <form method="POST">
                ...
                <div class="rmdash-actions">
                    ...
                    Settings</button>
                    <button class="rmdash-btn secondary" name="action" value="make_inclusive_mutual" type="submit">Make
                    Includes Mutual</button>
                    <button class="rmdash-btn secondary" name="action" value="make_exclusive_mutual" type="submit">Make
    ...
    def _dashboard_reaction_roles_section(
        self,
        guild: discord.Guild,
        react_roles: dict[str, dict[str, typing.Any]],
        csrf: str,
    ) -> str:
        rows = []
    ...
    def _dashboard_temporary_roles_section(
        ...
        for item in sorted(
            temp_roles,
            key=lambda data: float(data.get("expires_at", 0)),
        ):
    ...
    def _dashboard_components_section(
        self,
        guild: discord.Guild,
        buttons: dict[str, dict[str, typing.Any]],
        select_options: dict[str, dict[str, typing.Any]],
        select_menus: dict[str, dict[str, typing.Any]],
        csrf: str,
    ) -> str:
        button_rows = []
        button_editors = []
        for name, data in sorted(buttons.items()):
            ...
            style_name = {
                ...
                int(data.get("style") or discord.ButtonStyle.secondary.value),
                "secondary",
            )
