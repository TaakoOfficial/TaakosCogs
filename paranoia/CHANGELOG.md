...
- Renamed the repository package folder from `Paranoia` to `paranoia` to match Red cog naming standards.
- New installs should use `[p]cog install taakoscogs paranoia` and `[p]load paranoia`.
- Existing Paranoia settings and game data are preserved because the cog's Config identifier did not change.
...
- Added native `/paranoia` commands with a slash-safe start command supporting up to ten players.
