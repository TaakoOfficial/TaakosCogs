...
class EmojiPorter(commands.Cog):
    ...
    async def _check_permissions(
        ...
        if (
            manage_emojis
            and not ctx.guild.me.guild_permissions.manage_emojis_and_stickers
        ):
    ...
    async def _copy_emoji(
        ...
        try:
            ...
            return await target_guild.create_custom_emoji(
    ...
    async def _copy_sticker(
        self,
        source_sticker: discord.GuildSticker,
        target_guild: discord.Guild,
        ...
        try:
            ...
            return await target_guild.create_sticker(
                name=source_sticker.name,
    ...
    @commands.hybrid_command(
        name="copyemojis",
        description="Copy emojis from source server to current server.",
    )
    @commands.guild_only()
    async def copyemojis(
        self,
        ctx: commands.Context,
        source_guild_id: int,
        emoji_names: str | None = None,
    ):
        """
        ...
        if emoji_names:
            ...
            emojis_to_copy = [
                emoji for emoji in source_guild.emojis if emoji.name in requested_names
            ]
        ...
        progress_msg = await ctx.send(
            f"🔄 Starting to copy {len(emojis_to_copy)} emoji(s)...",
        )
    ...
    @commands.hybrid_command(
        name="copystickers",
        description="Copy stickers from source server to current server.",
    )
    @commands.guild_only()
    async def copystickers(
        self,
        ctx: commands.Context,
        source_guild_id: int,
        sticker_names: str | None = None,
    ):
        """
        ...
        if sticker_names:
            ...
            stickers_to_copy = [
                ...
                if sticker.name in requested_names
            ]
        ...
        # Send initial message
        progress_msg = await ctx.send(
            f"🔄 Starting to copy {len(stickers_to_copy)} sticker(s)...",
        )
    ...
    @commands.hybrid_command(
        name="listemojis",
        description="List all emojis in a server.",
    )
    @commands.guild_only()
    async def listemojis(self, ctx: commands.Context, guild_id: int | None = None):
        ...
        if not target_guild.emojis:
            await ctx.send(
                f"📭 No custom emojis found in **{target_guild.name}**.",
                ephemeral=bool(ctx.interaction),
            )
            return
    ...
    async def liststickers(self, ctx: commands.Context, guild_id: int | None = None):
        ...
        if not target_guild.stickers:
            await ctx.send(
                f"📭 No custom stickers found in **{target_guild.name}**.",
                ephemeral=bool(ctx.interaction),
            )
            return
