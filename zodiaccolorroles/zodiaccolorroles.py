...
class ZodiacColorRoles(commands.Cog):
    ...
    async def _create_role_safely(
        ...
        try:
            ...
            raise commands.CommandError(
                f"Failed to create role due to Discord API error: {e}",
            )

    @commands.hybrid_command(
        name="listzodiacroles",
        description="List all available zodiac roles.",
    )
    async def listzodiacroles(self, ctx: commands.Context):
        ...
        await ctx.send(
            f"Available zodiac roles: {zodiac_list}",
            ephemeral=bool(ctx.interaction),
        )

    @commands.hybrid_command(
        name="listcolorroles",
        description="List all available color roles.",
    )
    async def listcolorroles(self, ctx: commands.Context):
        """List all color roles the bot can create."""
        color_list = ", ".join(
            f"{name} ({hexcode})" for name, hexcode in self.COLOR_CHOICES.items()
        )
        await ctx.send(
            f"Available color roles: {color_list}",
            ephemeral=bool(ctx.interaction),
        )

    @commands.hybrid_command(
        name="addzodiacrole",
        description="Create a zodiac role for a user, or all zodiac roles.",
    )
    async def addzodiacrole(self, ctx: commands.Context, zodiac: str):
        ...
        if zodiac.lower() == "all":
            ...
            if added_roles:
                await ctx.send(
                    f"Added zodiac roles: {', '.join(added_roles)}",
                    ephemeral=bool(ctx.interaction),
                )
            if failed_roles:
                await ctx.send(
                    f"Failed to create: {', '.join(failed_roles)}",
                    ephemeral=bool(ctx.interaction),
                )
            return
        ...
        if zodiac not in self.ZODIAC_SIGNS:
            await ctx.send(
                "Invalid zodiac sign. Use `/listzodiacroles` to see valid options.",
                ephemeral=bool(ctx.interaction),
            )
            return
        ...
        if not role:
            ...
            await ctx.send(
                f"Zodiac role already exists: {role_name}",
                ephemeral=bool(ctx.interaction),
            )

    @commands.hybrid_command(
        name="addcolorrole",
        description="Create a color role for a user, or all color roles.",
    )
    async def addcolorrole(self, ctx: commands.Context, color: str):
        ...
        if color_name.lower() == "all":
            ...
            if added_roles:
                await ctx.send(
                    f"Added color roles: {', '.join(added_roles)}",
                    ephemeral=bool(ctx.interaction),
                )
            if failed_roles:
                await ctx.send(
        ...
        if color_name not in self.COLOR_CHOICES:
            await ctx.send(
                "Invalid color. Use `/listcolorroles` to see valid options.",
                ephemeral=bool(ctx.interaction),
            )
            return
        ...
        if not role:
            ...
            await ctx.send(
                f"Color role already exists: {role_name}",
                ephemeral=bool(ctx.interaction),
            )

    @commands.hybrid_command(
        name="addpronounrole",
        description="Create a pronoun role or all pronoun roles on the server.",
    )
    async def addpronounrole(self, ctx: commands.Context, pronoun: str):
        ...
        if pronoun_name == "all":
            ...
            if added_roles:
                await ctx.send(
                    f"Added pronoun roles: {', '.join(added_roles)}",
                    ephemeral=bool(ctx.interaction),
                )
            if failed_roles:
                await ctx.send(
        ...
        if pronoun_name not in valid_names:
            await ctx.send(
                f"Invalid pronoun. Valid options: {', '.join(self.PRONOUN_ROLES)}",
                ephemeral=bool(ctx.interaction),
            )
            return
        ...
        if not role:
            try:
                role = await self._create_role_safely(guild, role_name)
                await ctx.send(
                    f"Added pronoun role: {role_name}",
                    ephemeral=bool(ctx.interaction),
                )
            except (commands.BotMissingPermissions, commands.CommandError) as e:
                await ctx.send(
            ...
            await ctx.send(
                f"Pronoun role already exists: {role_name}",
                ephemeral=bool(ctx.interaction),
            )

    @commands.hybrid_command(
        name="addcommonpingrole",
        description="Create a common ping role or all common ping roles on the server.",
    )
    async def addcommonpingrole(self, ctx: commands.Context, pingrole: str):
        ...
        if pingrole_name == "all":
            ...
            if added_roles:
                await ctx.send(
                    f"Added common ping roles: {', '.join(added_roles)}",
                    ephemeral=bool(ctx.interaction),
                )
            if failed_roles:
                await ctx.send(
        ...
        if pingrole_name not in valid_names:
            await ctx.send(
                f"Invalid common ping role. Valid options: {', '.join(self.COMMON_PING_ROLES)}",
                ephemeral=bool(ctx.interaction),
            )
            return
        ...
        if not role:
            try:
                role = await self._create_role_safely(guild, role_name)
                await ctx.send(
                    f"Added common ping role: {role_name}",
                    ephemeral=bool(ctx.interaction),
                )
            except (commands.BotMissingPermissions, commands.CommandError) as e:
                await ctx.send(
            ...
            await ctx.send(
                f"Common ping role already exists: {role_name}",
                ephemeral=bool(ctx.interaction),
            )
