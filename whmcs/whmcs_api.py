...
class WHMCSAPIClient:
    ...
    async def get_ticket(self, ticket_id: str) -> dict[str, Any]:
        ...
        log.info(
            f"WHMCS API: Looking up ticket {clean_ticket_id} using only ticketnum parameter",
        )
        try:
            getticket_params = {
    ...
    async def add_ticket_reply(
        ...
        if id_field and id_field == "ticketid":
            ...
            parameters["ticketid"] = clean_ticket_id
            log.info(
                f"WHMCS API: Using numeric ticket_id as ticketid: {clean_ticket_id}",
            )
        else:
            ...
            try:
                log.info(
                    f"WHMCS API: Looking up ticket {clean_ticket_id} to get internal ID",
                )
                ...
                if ticket_resp.get("ticket"):
                    ...
                    if internal_id:
                        ...
                        log.info(
                            f"WHMCS API: Resolved ticket ID {clean_ticket_id} to internal ID {internal_id}",
                        )
                    else:
                        log.error(
                            f"WHMCS API: Could not find internal ticket ID for {clean_ticket_id}",
                        )
                        return {
                            "result": "error",
                            "message": f"Could not find internal ticket ID for {clean_ticket_id}",
                        }
                else:
        ...
        try:
            ...
            log.error(
                f"WHMCS API: AddTicketReply failed with parameters {parameters}: {e}",
            )
