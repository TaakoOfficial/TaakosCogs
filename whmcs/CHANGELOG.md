...
- Renamed the repository package folder from `WHMCS` to `whmcs` to match Red cog naming standards.
- New installs should use `[p]cog install taakoscogs whmcs` and `[p]load whmcs`.
- Existing WHMCS settings are preserved because the cog's Config identifier did not change.
