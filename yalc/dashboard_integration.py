...
class DashboardIntegration:
    ...
    async def _yd_save_filters(self, guild, form) -> None:
        ...
        await conf.tupperbox_ids.set([str(item) for item in self._yd_id_lines(form, "tupperbox_ids", maximum=50)])
...
{core}{events}{filters}{rules}</section>"""
    ...
    def _yd_filter_form(self, guild, settings, csrf):
        ...
        return f"""<form method="POST" class="card">{csrf}<input type="hidden" name="action" value="save_filters"><h3>Broad ignores & proxy filters</h3><div class="grid"><label>Ignored user IDs<textarea name="ignored_users" placeholder="One user ID per line">{users}</textarea></label><label>Ignored roles<select name="ignored_roles" multiple>{role_options}</select></label><label>Ignored channels<select name="ignored_channels" multiple>{channel_options}</select></label><label>Ignored categories<select name="ignored_categories" multiple>{category_options}</select></label><label>Proxy bot IDs<textarea name="tupperbox_ids" placeholder="One bot ID per line">{tupper}</textarea></label><label>Proxy command prefixes<textarea name="message_prefix_filter" placeholder="One prefix per line">{prefixes}</textarea></label><label>Webhook name filters<textarea name="webhook_name_filter" placeholder="One partial name per line">{webhooks}</textarea></label></div><button class="btn btn-primary">Save Filters</button></form>"""
