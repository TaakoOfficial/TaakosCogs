...
- Renamed the repository package folder from `Fable` to `fable` to match Red cog naming standards.
- New installs should use `[p]cog install taakoscogs fable` and `[p]load fable`.
- Existing Fable settings and records are preserved because the cog's Config identifier did not change.
